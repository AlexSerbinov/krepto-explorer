...
const debugLog = debug("kreptoexp:rpc");
...
function getIndexInfo() {
	if (semver.gte(global.kreptoNodeSemver, minRpcVersions.getindexinfo)) {
...
function getDeploymentInfo() {
	if (semver.gte(global.kreptoNodeSemver, minRpcVersions.getdeploymentinfo)) {
...
function getBlockStats(hash) {
	if (semver.gte(global.kreptoNodeSemver, minRpcVersions.getblockstats)) {
...
async function noTxIndexTransactionLookup(txid, walletOnly) {
	...
	// This is only available in Electrs and requires enabling KREPTOEXP_ELECTRUM_TXINDEX.
